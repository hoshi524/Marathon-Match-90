...
public class RollingBallsVis {
	...
	double getScore() {
		...
		for (int i = 0; i < H; ++i)
			for (int j = 0; j < W; ++j)
				if (targetMaze[i][j] != '#' && targetMaze[i][j] != '.') if (curMaze[i][j] == targetMaze[i][j]) {
					score += 1;
	...
	public class Vis extends JPanel implements MouseListener, WindowListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (firstClick) {
				if (curMaze[p.r][p.c] == '.' || curMaze[p.r][p.c] == '#') return;
			...
			for (int dir = 0; dir < 4; ++dir)
				if (p.r == first.r + dr[dir] && p.c == first.c + dc[dir]) {
					...
					firstClick = true;
					++manualRolls;
					if (manualRolls == maxRolls) manualReady = true;
					repaint();
					return;
				}
		}
	...
	// -----------------------------------------
	public RollingBallsVis(long seed) {
		try {
			if (vis) {
				jf = new JFrame();
				v = new Vis();
